...
La perturbazione è dunque pari al **gradiente** della loss function $J(\theta,y,x)$ rispetto all'immagine di ingresso $x$.
...
Al fine di realizzare l'attacco target, si utilizza una variante del *FSGM*, che, invece di costruire una perturbazione che aumenti la loss $J(\theta,y,x)$, crea una perturbazione che minimizza la $J(\theta,y_{t},x)$.
...
Viene testata la tecnica *non target iterativa* su un'immagine del test set. Viene costruito un esempio avversario per la rete surrogata, verificandone l'efficacia nel far sbagliare la rete black box target.

Si noti che l'attacco è generato sulla rete **black box**, ma le prestazioni vengono valutate sulla **rete surrogata**. In particolare si effettuano le predizioni dell'array di label, sia dell'immagine originale che di quella perturbata, proprio sul modello black box creato ad hoc per questo scopo.
"""
...
fake_img = iterative_attack(non_targeted_attack, orig_img, imgs[1][0:10],network,10,0.15)
pred = bb_network.predict(orig_img)
fake_pred = bb_network.predict(fake_img)
